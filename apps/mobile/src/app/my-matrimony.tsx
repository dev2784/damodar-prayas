import { C, styles } from '@/styles/my-matrimony.styles';
...
function ProfileCard({
  ...
  onRequestDelete: (profile: MatrimonyOwnerProfile) => void;
}) {
    ...
    <View style={styles.card}>
      ...
      <View style={styles.metaRow}>
        <Text style={styles.metaText} numberOfLines={1}>
          {[profile.currentCity, profile.state].filter(Boolean).join(', ') ||
            'स्थान अभी नहीं जोड़ा'}
        </Text>
      ...
      {profile.status === 'APPROVED' ? (
        <View style={styles.approvedEditNote}>
          <Text style={styles.approvedEditText}>एडिट करने पर बदलाव दोबारा समीक्षा में जाएंगे।</Text>
        </View>
      ) : null}
        ...
        activeDeleteRequest ? (
          <View style={styles.deletePendingBox}>
            ...
            <View style={styles.flexFill}>
              <Text style={styles.deletePendingTitle}>प्रोफाइल हटाने का अनुरोध भेजा गया है</Text>
              <Text style={styles.deletePendingText} numberOfLines={2}>
                {activeDeleteRequest.reason}
              </Text>
            </View>
          </View>
        ) : (
          <Pressable
            ...
            style={[styles.deleteRequestButton, requestingDelete && styles.buttonDisabled]}
          ...
          >
            {requestingDelete ? (
              ...
              <>
                <SymbolView
                  name={{ ios: 'trash.slash', android: 'delete_forever', web: 'delete_forever' }}
                  tintColor={C.red}
                  size={15}
                />
                <Text style={styles.deleteRequestText}>प्रोफाइल हटाने का अनुरोध</Text>
              </>
            )}
          </Pressable>
        )
...
export default function MyMatrimonyScreen() {
  ...
  const { data, isLoading, isFetching, isError, refetch } = useGetMyMatrimonyProfilesQuery(
    undefined,
    { skip: !accessToken },
  );
  ...
  function openDeleteRequest(profile: MatrimonyOwnerProfile) {
  ...
  async function submitDeleteRequest() {
    ...
    if (reason.length < 10) {
      Alert.alert('कारण लिखें', 'प्रोफाइल हटाने का कारण कम से कम 10 अक्षरों में लिखें।');
      return;
    }
    try {
      setRequestingDeleteId(deleteTarget.id);
      await requestDeletion({ id: deleteTarget.id, reason }).unwrap();
      setDeleteTarget(null);
      setDeleteReason('');
      Alert.alert('अनुरोध भेज दिया', 'एडमिन समीक्षा के बाद प्रोफाइल हटाने की कार्रवाई करेगा।');
    } catch (error) {
      const code =
      ...
      Alert.alert(
        ...
        code === 'DELETE_REQUEST_ALREADY_PENDING'
          ? 'इस प्रोफाइल का deletion request पहले से pending है।'
      ...
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.authState}>
          ...
          <Text style={styles.authText}>
            आपकी ड्राफ्ट, संपर्क जानकारी और प्रोफाइल की स्थिति निजी रहती है, इसलिए पहले अपना अकाउंट
            सत्यापित करें।
          </Text>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.headerRow}>
          ...
          <Pressable
            ...
            onPress={() =>
              router.push({ pathname: '/matrimony-form', params: { newProfile: '1' } })
            }
          >
            <SymbolView
        ...
        <View style={styles.infoBanner}>
          ...
          <View style={styles.infoCopy}>
            ...
            <Text style={styles.infoText}>
              ड्राफ्ट सीधे हट सकता है। Submitted/approved प्रोफाइल हटाने के लिए admin request जाएगी।
            </Text>
          </View>
        </View>
        <Pressable style={styles.shortlistLink} onPress={() => router.push('/matrimony-shortlist')}>
          <View style={styles.shortlistIcon}>
            <SymbolView
          ...
          <View style={styles.shortlistCopy}>
            <Text style={styles.shortlistTitle}>मेरी Shortlist</Text>
            <Text style={styles.shortlistText}>आपके सेव किए हुए मैट्रिमोनी प्रोफाइल</Text>
          </View>
          <SymbolView
          ...
          <View style={styles.listBlock}>
            ...
            {data?.items.map((profile) => (
              <ProfileCard
                ...
                requestingDelete={requestingDeleteId === profile.id}
                onSubmit={handleSubmit}
                onDelete={handleDelete}
                onRequestDelete={openDeleteRequest}
              />
            ))}
          </View>
        )}
        ...
        <View style={styles.modalBackdrop}>
          <View style={styles.modalCard}>
            <View style={styles.modalIcon}>
              <SymbolView
                name={{ ios: 'trash.slash.fill', android: 'delete_forever', web: 'delete_forever' }}
                tintColor={C.red}
                size={26}
              />
            </View>
            <Text style={styles.modalTitle}>प्रोफाइल हटाने का अनुरोध</Text>
            <Text style={styles.modalText}>
              {deleteTarget
                ? `${nameOf(deleteTarget)} को हटाने का कारण बताइए। Admin review के बाद profile delete होगी।`
                : ''}
            </Text>
            <TextInput
              value={deleteReason}
              onChangeText={setDeleteReason}
              placeholder="उदाहरण: विवाह तय हो गया है / अब प्रोफाइल की आवश्यकता नहीं है"
              placeholderTextColor="#A69A94"
              multiline
              maxLength={1000}
              style={styles.reasonInput}
            />
            <Text style={styles.reasonCount}>{deleteReason.trim().length}/1000</Text>
            <View style={styles.modalActions}>
              <Pressable
                disabled={requestingDeleteId !== null}
              ...
              <Pressable
                disabled={requestingDeleteId !== null}
                style={[styles.modalSubmit, requestingDeleteId !== null && styles.buttonDisabled]}
                onPress={submitDeleteRequest}
              >
                {requestingDeleteId ? (
                  <ActivityIndicator color="#FFFFFF" size="small" />
                ) : (
                  <Text style={styles.modalSubmitText}>अनुरोध भेजें</Text>
                )}
              </Pressable>
            </View>
          </View>
